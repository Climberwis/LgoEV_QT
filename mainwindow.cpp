...
void MainWindow::play_game(){
    prepare_fields();
    for(int i=0; i<10000; i++){
        if((map[i]->v_value() == herb_v || map[i]->v_value() == carn_v) && map[i]->v_moved() == no){
            int j = losuj_pole(i);
            compare(map, i, j);
        }
    }
...
void MainWindow::prepare_fields(){
    for(int i=0; i<10000; i++){
        map[i]->set_moved(no);
        if (map[i]->v_value() == plant_v){
            map[i]->decr_life(2);
        } else if(map[i]->v_value() == herb_v){
            map[i]->decr_life(1);
        } else if(map[i]->v_value() == carn_v){
            map[i]->decr_life(4);
        }
        if( map[i]->v_life() <= 0 ) map[i]->set_field(land_v);
    }
   add_plant();
}
...
void MainWindow::display(){
    life->zero_field();
    for(int i=0; i<10000; i++){
       life->count_field(map[i]->v_value());
    }
